...
filepath = 'results/07-allele-list/' + accession + '.allele_list_fl.txt'
...
def fn_expand_thd(ipd_num_dict,
                  accession,
                  dirpath,
                  filename='{allele_fl}.fasta'):
    filepath='results/07-allele-list/' + accession + '.allele_list_fl.txt'
    with open(filepath) as f:
        # ) and (x in ipd_diag_dict.keys())
        allele_list = f.read().split('\n')
    # print(allele_list)
    allele_fl = [x.strip('\"')  for x in allele_list if len(x) > 0]
    #print(allele_fl)
    return expand(os.path.join(dirpath, filename), allele_fl=allele_fl)
...
rule exhaustive_mapping_fl:
    ...
    output:
        temp(fn_expand_thd(ipd_num_dict, accession, dirpath='results/07-exaustive_fl/', filename='{allele_fl}.sam'))
    threads: 1
    run:
...
rule sort_sam_fl:
    ...
    """
    input:
        fn_expand_thd(ipd_num_dict, accession, dirpath='results/07-exaustive_fl/', filename='{allele_fl}.sam'),
    output:
        temp(fn_expand_thd(ipd_num_dict, accession, dirpath='results/08-sort_fl/', filename='{allele_fl}.sorted.sam'))
    threads: 1
    run:
...
rule index_fasta_fl:
    ...
    """
    input:
        fn_expand_thd(ipd_num_dict, accession, dirpath=indvl_allele_fl_fa_dir, filename='{allele_fl}.fasta')
    output:
        fn_expand_thd(ipd_num_dict, accession, dirpath=indvl_allele_fl_fa_dir, filename='{allele_fl}.fasta.fai')
    threads: 1
    run:
...
rule convert_bam_fl:
    ...
    """
    input:
        fn_expand_thd(ipd_num_dict, accession, dirpath='results/08-sort_fl/', filename='{allele_fl}.sorted.sam'),
        fn_expand_thd(ipd_num_dict, accession, dirpath=indvl_allele_fl_fa_dir, filename='{allele_fl}.fasta'),
        fn_expand_thd(ipd_num_dict, accession, dirpath=indvl_allele_fl_fa_dir, filename='{allele_fl}.fasta.fai')
    output:
        temp(fn_expand_thd(ipd_num_dict, accession, dirpath='results/09-cv-bam-fl/', filename='{allele_fl}.sorted.bam'))
    run:
